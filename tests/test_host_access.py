...
async def test_stale_host_job_cannot_create_an_approval(db, conversation, tmp_path):
    settings = Settings(host_access_enabled=True)
...
async def test_host_command_is_private_owner_only(db, tmp_path):
    ...
    broker, token, _ = await broker_context(
        db, Settings(host_access_enabled=True), conversation, tmp_path
    )
    try:
