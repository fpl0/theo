"""Canonical memory revisions, reviewed corrections and current-only retrieval.

Owns memory history, archive/restore operations, evidence links and scoped search.
Context assembly and optional embedding model execution are separate services.
"""
...
from theo.privacy import label_in, visible_in
...
class Memory:
    ...
    def _get(self, db: sqlite3.Connection, memory_id: str) -> sqlite3.Row:
        ...
        if row is None or not visible_in(db, "memory", memory_id, self.scope):
    ...
    async def remember(
        ...
        *,
        ...
        def insert(db: sqlite3.Connection) -> str:
            ...
            label_in(db, "memory", memory_id, self.scope)
    ...
    async def search(self, query: str, limit: int = 20) -> list[Json]:
        ...
        return await self.db.read(
            "SELECT m.*,r.body,r.provenance,r.source,bm25(memory_fts) AS lexical_rank FROM memory_fts JOIN memory_records m ON m.id=memory_fts.memory_id JOIN memory_revisions r ON r.memory_id=m.id AND r.version=m.revision WHERE memory_fts MATCH ? AND m.owner_id=? AND m.status='active' AND (? IS NULL OR EXISTS(SELECT 1 FROM resource_scopes s WHERE s.kind='memory' AND s.resource_id=m.id AND s.conversation_id=?)) ORDER BY lexical_rank LIMIT ?",
            (fts, self.owner, self.scope, self.scope, min(max(limit, 1), 50)),
        )
    ...
    async def neighbours(self, ids: list[str], depth: int = 2) -> list[Json]:
        ...
        for _ in range(min(depth, 2)):
            ...
            for memory_id in ids[:50]:
                ...
                for row in rows:
                    if row["id"] not in seen and await self.db.write(
                        lambda connection, row=row: visible_in(
                            connection, "memory", row["id"], self.scope
                        )
                    ):
                        seen.add(row["id"])
                        found.append(row)
