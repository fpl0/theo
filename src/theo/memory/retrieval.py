...
# ---------------------------------------------------------------------------
#
# The ``kind`` filter is applied via ``str.replace()`` on ``_RRF_TEMPLATE``
# to avoid duplicating ~90 lines of SQL.  All other parameters use fixed
# positional indices.
#
# Fixed parameters:
...
#
# When *kind* is requested, $8 is the kind string, and the vector_ranked
# and fts_ranked CTEs add ``AND kind = $8`` to their WHERE clauses.
...
fts_ranked AS (
    ...
    FROM node
    WHERE tsv @@ plainto_tsquery('english', $3){kind_filter}
    ORDER BY ts_rank_cd(tsv, plainto_tsquery('english', $3)) DESC
    LIMIT $2
),
graph_seeds AS (
...
_HYBRID_SEARCH = _RRF_TEMPLATE.replace("{kind_filter}", "")
_HYBRID_SEARCH_BY_KIND = _RRF_TEMPLATE.replace("{kind_filter}", " AND kind = $8")
