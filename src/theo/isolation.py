"""OS boundaries; a working directory is not a sandbox."""
    ...
    *,
    ...
    if sys.platform == "darwin" and Path("/usr/bin/sandbox-exec").exists():
        profile = sandbox_profile(
            protected_root, settings.worker_home, workspace, generated=generated
        )
...
def sandbox_profile(
    ...
    *,
    ...
    profile = (
        ...
        f"(deny file-read* (require-all (subpath {quote(Path.home())}) (require-not (subpath {quote(runner)})) (require-not (subpath {quote(Path(sys.prefix))})) (require-not (subpath {quote(Path(__file__).resolve().parents[2])}))))"
        ...
        "(deny mach-priv*)(deny process-info*)(deny signal)"
    )
    if generated:
