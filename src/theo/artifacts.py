"""Immutable, validated artifact bytes; full extracted memory text stays in SQLite."""
...
class Artifacts:
    ...
    async def store(
        self, raw: bytes, name: str, description: str, run_id: str | None = None
    ) -> Json:
        mime, text = await asyncio.to_thread(validate_artifact, raw, name)
