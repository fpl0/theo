"""Record worker process birth identity for safe crash recovery.

Persists run-to-PID bindings and checks process creation time before terminating
an orphan, so a recycled PID is never treated as an owned worker.
"""
