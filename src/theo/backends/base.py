...
TOOL_CONTRACT = (
    ...
    "command_run and file_read are confined to the job workspace. For a host file, inspect "
    "get_status.host_access and use host_read on the operator-granted roots. A "
    "command_policy of standing means general and privileged host_command operations are "
    "already authorized: do not ask the owner to repeat that permission. Poll action_status "
    "for the completed result before interpreting it or continuing dependent work. A workspace "
    "permission denial does not establish that the host path is unreadable. Investigate "
    "available permitted routes before reporting a blocker; never bypass enforced grants. "
