...
```sh
colima start theo-observability --activate=false --cpu 2 --memory 3 --disk 20 --vm-type vz
...
Start with **Attention now** on the overview, then follow blocked work into Codex diagnostics or open an event's trace. All six dashboards use compact status cards, consistent outcome colors, descriptive legends and a “Reading the signals” guide. The default window is 24 hours. Navigation preserves both the time window and the **Traffic** selection: **Live Theo** is real activity; **Test fixtures** is isolated validation. Missing measurements remain **No samples** or **Awaiting run**, never an invented zero. The CLI log charts use five-minute windows. Infrastructure shows current physical memory and headroom against the decimal 4 GB ceiling separately from the last completed load test.

`check` also runs eleven Prometheus behavioral cases from `observability/tests/poller-cases.json` against the generated Telegram poller alert. They cover healthy, stopped, never-observed and unconfigured pollers, plus fixture traffic that must not hide a missing live poller. The rule retains the last poll time after its series becomes stale and still fires if no live sample exists in the preceding 24 hours.

`uv run --no-sync python scripts/check_alert_rules.py` runs all 25 synthetic Prometheus scenarios, including known-zero versus unavailable inventory, verified recovery, environment separation, component labels and the 4 GB threshold.

Twenty-one rules cover core and observer availability, stalled jobs/deliveries, uncertain effects, Telegram polling, auth/quota blocks, disk/memory pressure, container restarts, late schedules, SDK/collector losses and missing/stale telemetry. The independent native observer accepts minimal local webhook receipts at `/alerts`; those are diagnostic receipts, not a replacement for remote notifications.

To configure Telegram delivery, edit the private `observability/.env` locally and add `THEO_ALERT_BOT_TOKEN` and `THEO_ALERT_CHAT_ID` for your verified test bot and owner chat. The `up` command creates the file with a random Grafana password on first use; it does not discover a Telegram credential. Preserve `GRAFANA_ADMIN_PASSWORD` and mode 0600, then rerun `up` with the selected data root to apply the contact point. Provisioning references environment variables rather than committed credentials.

Routine local/test alerts go only to **Theo local diagnostics**, a local webhook receipt. They do not notify Telegram. The explicit test marker below still exercises Telegram delivery when credentials are configured. Production routing requires `environment=production`; set `THEO_ENVIRONMENT=production`, `THEO_ALERT_HOST=fpl0.local`, `THEO_ALERT_LABEL=THEO PRODUCTION`, and `THEO_GRAFANA_URL=http://fpl0.local:13000` in the private deployment environment.

Notifications group by environment, host and incident, combining related availability or monitoring failures. They include severity, component, observed value, recovery guidance and the configured LAN dashboard URL. Initial grouping waits 30 seconds for availability/delivery/monitoring, one minute for other production incidents; updates are grouped for five minutes. Unchanged incidents repeat every four hours for availability/delivery/monitoring and every twelve hours otherwise. Explicit notification tests use ten-second initial grouping and one-minute updates.

Operational incidents keep their last state on missing data or query errors. Separate monitoring rules detect an unreachable observer, stale snapshot, unreadable database or unavailable footprint; the observer rule also detects Prometheus evaluation failures. Absent job statuses become zero only with a complete, readable database snapshot. Healthy measurements must persist for two minutes before recovery. A missing component series is retained for ten minutes; if Grafana later expires it, the message says **STATE CHANGED**, with the reason and **recovery not verified**, rather than claiming recovery. The explicit test marker has immediate recovery.

Only one rule reports model-access blocks. These are native model-access conditions, not necessarily a Codex login problem. Preserve the job, inspect its actual error, and explicitly retry after recovery. No daily manual Codex attestation or metered fallback is introduced.
...
The five hard container limits sum to 1,728 MiB. The isolated VM has 3 GiB of guest RAM. The **whole observability budget is 4,000,000,000 bytes**, including the VM, Docker, native observer and host VM helpers.
...
The earlier 2 GB qualification used a 384 MiB Grafana limit after a 256 MiB allocation OOMed during provisioning reload. The current 768 MiB limit gives Grafana more headroom; its Go memory target is 480 MiB. Historical 2 GB reports do not qualify this new envelope. The budget qualification indicator requires a report for the current budget; use the live physical-footprint graph for current consumption.
